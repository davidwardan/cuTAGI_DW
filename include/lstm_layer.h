...
void lstm_cat_activations_and_prev_states(std::vector<float> &a,
                                          ...
                                          int time_step, std::vector<float> &c);
...
void lstm_hidden_state_mean_var(
    ...
    std::vector<float> &cov_o_tanh_c, int no, int start_idx, int end_idx,
    int seq_len, int time_step, std::vector<float> &mu_z,
...
class LSTM : public BaseLayer {
   public:
    ...
    LSTM(size_t input_size, size_t output_size, bool last_timestep = false,
