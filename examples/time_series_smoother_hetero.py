...
from pytagi.nn import SLSTM, OutputUpdater, Sequential, SLinear, EvenExp
...
def main(num_epochs: int = 50, batch_size: int = 1, sigma_v: float = 1):
    ...
    for epoch in pbar:
        ...
        for idx_sample, (x, y) in enumerate(batch_iter):
            ...
            # Update output layer (heteroscedastic: model predicts its own variance)
            ...
            # Extract only the first output (Z^O mean) — second is the predicted variance
            m_pred = m_pred[:1]
            ...
            # Add new prediction to mu_sequence (only Z^O mean)
    ...
    for x, y in test_batch_iter:
        ...
        # m_pred[0] = Z^O mean, m_pred[1] = aleatoric variance (from EvenExp)
        # v_pred[0] = epistemic variance of the mean prediction
        mu_preds.append(m_pred[0])
        var_epistemic.append(v_pred[0])
        var_aleatoric.append(m_pred[1])
        x_test.extend(x)
        y_test.extend(y)

    mu_preds = np.array(mu_preds)
    var_epistemic = np.array(var_epistemic)
    var_aleatoric = np.array(var_aleatoric)
    ...
    std_epistemic = normalizer.unstandardize_std(
        var_epistemic**0.5, train_dtl.x_std[output_col]
    )
    std_aleatoric = normalizer.unstandardize_std(
        var_aleatoric**0.5, train_dtl.x_std[output_col]
    )
    ...
    log_lik = metric.log_likelihood(
        prediction=mu_preds, observation=y_test, std=std_total
    )

    # Visualization with epistemic and aleatoric uncertainty
    x_time = test_dtl.dataset["date_time"][: len(y_test)]
    plt.figure(figsize=(12, 12))
    plt.title("Test Predictions", fontsize=1.1 * 28, fontweight="bold")
    plt.plot(x_time, y_test, "k", lw=3, label=r"$y_{true}$")
    plt.plot(x_time, mu_preds, "r", lw=3, label=r"$\mathbb{E}[Y^{'}]$")
    plt.fill_between(
        x_time,
        mu_preds - std_epistemic,
        mu_preds + std_epistemic,
        color="r",
        alpha=0.3,
    ...
    plt.fill_between(
        x_time,
        mu_preds - std_aleatoric,
        mu_preds + std_aleatoric,
        color="g",
        alpha=0.3,
        label=r"Aleatoric $\pm 1\sigma$",
    )
    plt.legend(
        loc="upper right",
        edgecolor="black",
