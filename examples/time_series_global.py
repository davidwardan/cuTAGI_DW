...
class States:
    ...
    def update(self, new_mu, new_std, indices, time_step):
        ...
        Args:
            ...
            new_var: Array of new variance values.
...
def train_global_model(config, experiment_name: Optional[str] = None):
    ...
    # TODO: Initialize Embeddings Class Here if needed
    ...
    for epoch in pbar:
        ...
        for x, y, ts_id, w_id in train_batch_iter:
            ...
            batch_mse = metric.rmse(y_masked, m_pred_masked)
            batch_log_lik = metric.log_likelihood(
                y_masked, m_pred_masked, s_pred_masked
            )

            if not (np.isnan(batch_mse) and np.isnan(batch_log_lik)):
                train_mse += batch_mse * len(y_masked)
                train_log_lik += batch_log_lik * len(y_masked)
                count += len(y_masked)
        ...
        for x, y, ts_id, w_id in val_batch_iter:
            ...
            if not (np.isnan(batch_mse) and np.isnan(batch_log_lik)):
                val_mse += batch_mse * len(y_masked)
                val_log_lik += batch_log_lik * len(y_masked)
                count += len(y_masked)
    ...
    # get the true values for train, val, test
    np.savez(
        os.path.join(output_dir, "true_train.npz"),
        true_values=train_dtl.True_output,
    )
    np.savez(
        os.path.join(output_dir, "true_val.npz"),
        true_values=val_dtl.True_output,
    )
    np.savez(
        os.path.join(output_dir, "true_test.npz"),
        true_values=test_dtl.True_output,
    )
...
def eval_global_model(config, experiment_name: Optional[str] = None):
    ...
    true_train = np.load(input_dir / "true_train.npz")["true_values"]
    true_val = np.load(input_dir / "true_val.npz")["true_values"]
    true_test = np.load(input_dir / "true_test.npz")["true_values"]
    ...
    if config.eval_metrics:
        overall_rmse = np.mean(test_rmse_list)
        overall_log_lik = np.mean(test_log_lik_list)
        overall_mse = np.mean(test_mse_list)
        overall_p50 = np.mean(test_p50_list)
        overall_p90 = np.mean(test_p90_list)
        overall_mase = np.mean(test_mase_list)
...
def main(Train=False, Eval=True):
    ...
    experiment_name = "global_lstm_hq_B127_E10"
